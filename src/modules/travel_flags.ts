// Generated Flag Tile Sheet Interface
// Auto-generated by flag_tilesheet_generator.py

export interface FlagData {
  name: string;
  code: string;
  x: number;
  y: number;
  width: number;
  height: number;
}

export class TravelFlags {
  public static readonly TILESHEET_WIDTH = 120;
  public static readonly TILESHEET_HEIGHT = 20;
  public static readonly FLAG_WIDTH = 15;
  public static readonly FLAG_HEIGHT = 10;

  public static readonly FLAGS: FlagData[] = [
    { name: "Australia", code: "au", x: 0, y: 0, width: 15, height: 10 },
    { name: "United States", code: "us", x: 15, y: 0, width: 15, height: 10 },
    { name: "Japan", code: "jp", x: 30, y: 0, width: 15, height: 10 },
    { name: "Vietnam", code: "vn", x: 45, y: 0, width: 15, height: 10 },
    { name: "Maldives", code: "mv", x: 60, y: 0, width: 15, height: 10 },
    { name: "Italy", code: "it", x: 75, y: 0, width: 15, height: 10 },
    { name: "France", code: "fr", x: 90, y: 0, width: 15, height: 10 },
    { name: "Germany", code: "de", x: 105, y: 0, width: 15, height: 10 },
    { name: "Spain", code: "es", x: 0, y: 10, width: 15, height: 10 },
    { name: "United Kingdom", code: "gb", x: 15, y: 10, width: 15, height: 10 },
    { name: "Netherlands", code: "nl", x: 30, y: 10, width: 15, height: 10 },
    { name: "Switzerland", code: "ch", x: 45, y: 10, width: 15, height: 10 },
    { name: "Morocco", code: "ma", x: 60, y: 10, width: 15, height: 10 },
    { name: "Egypt", code: "eg", x: 75, y: 10, width: 15, height: 10 },
  ];

  public static getFlagByCode(code: string): FlagData | undefined {
    return this.FLAGS.find((flag) => flag.code === code);
  }

  public static getFlagByName(name: string): FlagData | undefined {
    return this.FLAGS.find(
      (flag) => flag.name.toLowerCase() === name.toLowerCase()
    );
  }
}

// EXAMPLE USAGE
// import { TravelFlags } from './travel_flags';

// // Get Australia flag coordinates
// const aussieFlag = TravelFlags.getFlagByCode('au');
// // Use aussieFlag.x, aussieFlag.y to render from the tile sheet
